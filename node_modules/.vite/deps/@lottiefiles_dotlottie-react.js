...
var e3 = class g {
  ...
  get layout() {
    ...
    if (i) return { align: [i.align.get(0), i.align.get(1)], fit: (() => {
      switch (i.fit) {
        case g._wasmModule?.Fit.Contain:
          return "contain";
        case g._wasmModule?.Fit.Cover:
          return "cover";
        case g._wasmModule?.Fit.Fill:
          return "fill";
        case g._wasmModule?.Fit.FitHeight:
          return "fit-height";
        case g._wasmModule?.Fit.FitWidth:
          return "fit-width";
        case g._wasmModule?.Fit.None:
          return "none";
        default:
          return "contain";
      }
  ...
  get mode() {
    ...
    return i === g._wasmModule?.Mode.Reverse ? "reverse" : i === g._wasmModule?.Mode.Bounce ? "bounce" : i === g._wasmModule?.Mode.ReverseBounce ? "reverse-bounce" : "forward";
  }
  get isFrozen() {
  ...
  static transformThemeToLottieSlots(i, n) {
    return g._wasmModule?.transformThemeToLottieSlots(i, n) ?? "";
  }
...
var g2 = class r1 {
  constructor(i) {
    ...
    this._worker = r1._workerManager.getWorker(n), r1._workerManager.assignAnimationToWorker(this._id, n), r1._wasmUrl && this._sendMessage("setWasmUrl", { url: r1._wasmUrl }), this._create({ ...i, renderConfig: { ...i.renderConfig, devicePixelRatio: i.renderConfig?.devicePixelRatio || d1(), freezeOnOffscreen: i.renderConfig?.freezeOnOffscreen ?? true } }), this._worker.addEventListener("message", this._handleWorkerEvent.bind(this));
  }
...
var K1 = ({ animationId: h, autoplay: i, backgroundColor: n, className: d, createDotLottie: _, data: w, dotLottieRefCallback: u, layout: W, loop: V, mode: J, playOnHover: p1, renderConfig: u1, segment: D, speed: c1, src: q, stateMachineConfig: i1, stateMachineId: K, style: l1, themeData: _1, themeId: z, useFrameInterpolation: E, workerId: n1, ...Z }) => {
  ...
  let k1 = (0, import_react.useCallback)((x) => {
    C.current = x, x ? L.current = _({ ...s1.current, canvas: x }) : (L.current?.destroy(), L.current = null), g1.current?.(L.current);
  }, []);
  return (0, import_react.useEffect)(() => {
    let x = (M1) => {
      p1 && (M1.type === "mouseenter" && L.current?.play(), M1.type === "mouseleave" && L.current?.pause());
    };
    ...
    L.current?.setSpeed(c1 ?? 1);
  }, [c1]), (0, import_react.useEffect)(() => {
    L.current?.setMode(J ?? "forward");
  }, [J]), (0, import_react.useEffect)(() => {
    L.current?.setLoop(V ?? false);
  }, [V]), (0, import_react.useEffect)(() => {
    L.current?.setUseFrameInterpolation(E ?? true);
  }, [E]), (0, import_react.useEffect)(() => {
    typeof D?.[0] == "number" && typeof D[1] == "number" && L.current?.setSegment(D[0], D[1]);
  }, [D]), (0, import_react.useEffect)(() => {
    L.current?.setBackgroundColor(n ?? "");
  }, [n]), (0, import_react.useEffect)(() => {
    L.current?.setRenderConfig(u1 ?? {});
  }, [JSON.stringify(u1)]), (0, import_react.useEffect)(() => {
    typeof w != "string" && typeof w != "object" || L.current?.load({ data: w, ...s1.current });
  }, [w]), (0, import_react.useEffect)(() => {
    typeof q == "string" && L.current?.load({ src: q, ...s1.current });
  }, [q]), (0, import_react.useEffect)(() => {
    L.current?.setMarker(Z.marker ?? "");
  }, [Z.marker]), (0, import_react.useEffect)(() => {
    L.current?.loadAnimation(h ?? "");
  }, [h]), (0, import_react.useEffect)(() => {
    typeof z == "string" && L.current?.setTheme(z);
  }, [z]), (0, import_react.useEffect)(() => {
    L.current?.setThemeData(_1 ?? "");
  }, [_1]), (0, import_react.useEffect)(() => {
    L.current?.setLayout(W ?? {});
  }, [W?.fit, W?.align && W.align[0], W?.align && W.align[1]]), (0, import_react.useEffect)(() => {
    L.current?.isLoaded && (typeof K == "string" && K ? L.current.stateMachineLoad(K) && L.current.stateMachineStart() : L.current.stateMachineStop());
  }, [K]), (0, import_react.useEffect)(() => {
    L.current?.stateMachineSetConfig(i1 ?? null);
